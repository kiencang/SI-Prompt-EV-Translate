...
4.  **Tái tạo Tài liệu Kỹ thuật số Nâng cao (PDF sang HTML/CSS)**:
	...
	*   **CSS Framework Cơ bản (Bắt buộc):**
		*   Trong `<style>`, bọc toàn bộ nội dung bằng một container có `max-width` (VD: 800px hoặc 900px), căn giữa `margin: 0 auto`, và thiết lập `line-height: 1.6` để tối ưu cho việc đọc bài báo dài.
		*   Thêm quy tắc chống tràn: `img, table { max-width: 100%; height: auto; overflow-x: auto; display: block; }`.
...
5.  **Xử lý Biểu thức và Công thức Toán học:**
	...
	*   **Xử lý Đồ họa Toán học Đơn giản (như Trục số - Number lines):**
		*   Đừng cố dùng CSS phức tạp để vẽ trục số.
		*   Ưu tiên dùng mã **SVG (Scalable Vector Graphics)** nhúng trực tiếp vào HTML để vẽ lại các trục số, điểm tròn đặc (solid circle), điểm tròn rỗng (hollow circle) và mũi tên. Điều này đảm bảo tính thẩm mỹ và không gây vỡ layout (đáp ứng Ưu tiên #3).
...
7.  **Xử lý Bố cục chung của Tài liệu:**			
...
8.  **Tính Nhất quán (Consistency):** Duy trì sự đồng nhất nghiêm ngặt về thuật ngữ, giọng văn, cách diễn đạt, và cách xử lý các yếu tố lặp lại (cả về dịch thuật và định dạng HTML/CSS) trong toàn bộ tài liệu.
