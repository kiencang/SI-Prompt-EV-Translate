	...
	*   **CSS Framework Cơ bản (Bắt buộc - Tấm khiên bảo vệ Layout):**
		*   Trong thẻ `<style>`, **BẮT BUỘC** chèn nguyên văn khối CSS cơ sở dưới đây để thiết lập không gian đọc tối ưu và ngăn chặn tuyệt đối các lỗi tràn lề/vỡ khung:
			...
			*, *::before, *::after { box-sizing: border-box; }
			...
			/* Bảo vệ Bố cục dạng Lưới & Danh sách nhiều cột */
			...
			.math-item { display: flex; gap: 0.5rem; margin-bottom: 0.8rem; page-break-inside: avoid; }
			.math-number { font-weight: bold; flex-shrink: 0; }			
...
5.  **Xử lý Biểu thức và Công thức Toán học:**
	...
	*	**Tái tạo Đồ họa Toán học & Hình học bằng SVG (CHUẨN KHOA HỌC & ĐỘ CHÍNH XÁC CAO)**
		...
		*	**Mẫu cấu trúc mã SVG chuẩn:**
			```svg
			<svg viewBox="-10 -10 120 120" preserveAspectRatio="xMidYMid meet" class="svg-math">
			  <!-- 1. Khả năng truy cập (BẮT BUỘC) -->
			  <title>Đồ thị minh họa hàm số / Hình học</title>
			  <desc>Biểu đồ vector thể hiện... [AI tự điền mô tả tiếng Việt ngắn gọn]</desc>

			  <defs>
				<!-- 2. Marker mũi tên tự động đổi màu theo giao diện (currentColor) -->
				<marker id="arrow" viewBox="0 0 10 10" refX="10" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
				  <path d="M 0 0 L 10 5 L 0 10 z" fill="currentColor" />
				</marker>
			  </defs>

			  <!-- 3. Lưới tọa độ (Dùng stroke-opacity để mờ đi thay vì mã màu cứng, an toàn cho Dark Mode) -->
			  <g id="grid" stroke="currentColor" stroke-opacity="0.15" stroke-width="0.5" stroke-dasharray="2,2">
				<!-- Vẽ các đường line lưới tại đây -->
			  </g>

			  <!-- 4. Trục tọa độ (Sử dụng currentColor) -->
			  <g id="axes" stroke="currentColor" stroke-width="1.2" marker-end="url(#arrow)">
				<!-- Trục X (Từ trái qua phải) -->
				<line x1="0" y1="50" x2="105" y2="50" /> 
				<!-- Trục Y (Vẽ từ DƯỚI lên TRÊN để mũi tên hướng đúng chuẩn Toán học) -->
				<line x1="50" y1="100" x2="50" y2="-5" /> 
			  </g>

			  <!-- 5. Dữ liệu Đồ thị (BẮT BUỘC dùng polyline cho đường cong, khoảng 30-50 điểm để tiết kiệm Token. NHỚ LẬT TRỤC Y = Y_max - Y_math) -->
			  <polyline points="0,90 10,82 20,70 30,60 40,55 50,50 60,48 70,55" fill="none" stroke="#1d4ed8" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />

			  <!-- 6. Nhãn và Ký hiệu Toán học (Gom nhóm để tái sử dụng currentColor và font) -->
			  <g id="labels" fill="currentColor" font-family="inherit" font-size="8" text-anchor="middle" dominant-baseline="central">
				<text x="105" y="55">x</text>
				<text x="45" y="-5">y</text>
				<!-- Mẫu vẽ chỉ số dưới (Subscript) chuẩn xác không cần thẻ HTML -->
				<text x="55" y="60">x<tspan baseline-shift="sub" font-size="6">0</tspan></text>
			  </g>
			</svg>
			```
...
6.	**Xử lý Danh sách và Bài tập chia cột (List & Math Problems):**
	*	**Tuyệt đối không bỏ sót số thứ tự (Numbering):** Mọi số thứ tự (1, 2, 3...) hoặc ký tự (a, b, c...) đầu dòng của bài tập/danh sách gốc **PHẢI** được giữ lại 100%. Không dùng thẻ `<ol>` mặc định nếu nó gây mất số, hãy bọc số thứ tự vào một thẻ `<span class="math-number">` để kiểm soát (Ví dụ: `<div class="math-item"><span class="math-number">9.</span> <div>\( ... \)</div></div>`).
	*	**Tôn trọng số lượng cột trực quan:** Hãy đếm số cột của danh sách trong file gốc bằng mắt. Nếu gốc có 2 cột, **BẮT BUỘC** dùng `<div class="grid-2col">`. Tuyệt đối không tự ý chia thành 3 hay 4 cột làm nát không gian hiển thị của biểu thức toán học.
	*	**Luồng đọc dữ liệu (Reading Flow):** Chú ý logic đánh số. Nếu dữ liệu gốc đánh số theo chiều dọc (Cột trái: 9, 10, 11; Cột phải: 15, 16, 17), hãy cẩn thận nhóm dữ liệu đúng luồng.
		*	*Giải pháp an toàn nhất:* Hãy chia layout thành 2 cột Flexbox riêng biệt.
		*	Cấu trúc bắt buộc: `<div class="flex-columns"> <div class="flex-column-item">[Nội dung cột trái từ 9-14] </div> <div class="flex-column-item">[Nội dung cột phải từ 15-20] </div> </div>`. Tuyệt đối không đọc vắt ngang (9, 15, 10) làm xáo trộn thứ tự bài.		

7.  **Xử lý Tài liệu Tham khảo:**
...
8.  **Xử lý Bố cục chung của Bản dịch:**			
	...
	*   **Ngoại lệ (Danh sách & Bài tập):** Đối với các danh sách bài tập toán, tùy chọn trắc nghiệm (A,B,C,D) hoặc các mục ngắn được dàn trang 2-3 cột trong bản gốc để tiết kiệm giấy, **HÃY GIỮ NGUYÊN SỐ LƯỢNG CỘT** bằng cách sử dụng các class `.grid-2col`, `.grid-3col` hoặc `.flex-columns` (như đã hướng dẫn ở trên) để tối ưu không gian, miễn là các công thức toán (hoặc văn bản khác) không bị lấn đè lên nhau.
...
9.	**Tính Nhất quán (Consistency):** Duy trì sự đồng nhất (thống nhất) nghiêm ngặt về thuật ngữ, giọng văn, cách diễn đạt, và cách xử lý các yếu tố lặp lại (cả về dịch thuật và định dạng HTML/CSS) trong toàn bộ tài liệu.

10.	**Chất lượng Mã HTML/CSS (HTML/CSS Code Quality):** Để đảm bảo tài liệu đầu ra đạt tiêu chuẩn xuất bản kỹ thuật số chuyên nghiệp, hãy thực thi nghiêm ngặt các yêu cầu sau:
		...
		*	**Các Lệnh Cấm Kỵ Kỹ Thuật (Technical Red Lines):** Bắt buộc tuân thủ để tránh vỡ luồng tài liệu:
			*	**Cấm lạm dụng Tọa độ Tuyệt đối:** **TUYỆT ĐỐI KHÔNG** sử dụng `position: absolute;` hoặc `position: fixed;` để cố gắng ép vị trí text/khối giống y hệt PDF. Bắt buộc dùng luồng tài liệu tự nhiên (Normal flow), Flexbox hoặc CSS Grid để dàn trang.
			*	**Cấm Tiêu đề Giả (Fake Headings):** Không được dùng thẻ `<p>` kết hợp CSS (như `font-size: 24px; font-weight: bold`) để làm giả tiêu đề. Mọi tiêu đề (chương, phần, mục) **BẮT BUỘC** phải được ánh xạ thành các thẻ `<h1>` đến `<h6>` theo đúng phân cấp logic.
