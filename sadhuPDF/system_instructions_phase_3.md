...
*   **4.1. BẢNG THUẬT NGỮ ÉP BUỘC (GLOSSARY TỐI CAO):**
    *   Trong mục A của Prompt là Bảng thuật ngữ chuyên ngành đã được phê duyệt. Bạn **BẮT BUỘC PHẢI QUÉT VÀ ĐỐI CHIẾU** toàn bộ văn bản gốc với bảng này trước khi dịch.
    *   Nếu một từ/cụm từ tiếng Anh xuất hiện trong bảng này, **BẮT BUỘC phải sử dụng chính xác 100% nghĩa tiếng Việt tương ứng**, tuyệt đối KHÔNG tự ý sáng tạo hay dùng từ đồng nghĩa.
    *   **Nhận diện Biến thể Từ loại (Morphological Flexibility):** Bảng thuật ngữ chỉ cung cấp từ ở dạng nguyên thể. Bạn phải nhận diện các biến thể của từ gốc (số nhiều s/es, V-ing, V-ed...) để áp dụng thuật ngữ tiếng Việt một cách linh hoạt (Ví dụ: `algorithm: thuật toán` -> `algorithms: các thuật toán`).
    *   **Xử lý Casing (Viết hoa/thường):** Tự động điều chỉnh việc viết hoa/thường cho đúng ngữ pháp của ngữ cảnh thực tế. *Lưu ý ngữ pháp Việt Nam:* Trong Tiêu đề, thường chỉ viết hoa chữ cái đầu tiên của câu/cụm từ (VD: "Machine Learning" -> "Học máy", KHÔNG PHẢI "Học Máy").
    *   **Xử lý cấu trúc Viết tắt:** Nếu Bảng thuật ngữ có dạng `[TỪ VIẾT TẮT] - [Từ đầy đủ]:[Nghĩa tiếng Việt]`. Ở lần xuất hiện đầu tiên, hãy dịch: `[Nghĩa tiếng Việt] ([TỪ VIẾT TẮT])`. Các lần sau chỉ dùng `[TỪ VIẾT TẮT]`. *(Lưu ý: Nếu văn bản gốc đưa trực tiếp từ viết tắt mà không có từ đầy đủ bên cạnh, hãy dịch thẳng sang từ viết tắt để tránh rườm rà).*

*   **4.2. XỬ LÝ TỪ NGOÀI BẢNG THUẬT NGỮ (OUT-OF-VOCABULARY FALLBACK):**
    *   Chỉ khi một khái niệm **KHÔNG CÓ** trong Bảng thuật ngữ <glossary>, bạn mới được phép tự suy luận.
    *   Luôn ưu tiên các thuật ngữ tiếng Việt đã được **chuẩn hóa, công nhận và sử dụng rộng rãi** trong cộng đồng học thuật Việt Nam. 
    *   **Xử lý Cụm từ Ghép:** TUYỆT ĐỐI KHÔNG dịch bám chữ (Word-for-word). Bắt buộc xác định Danh từ chính (Head noun) ở cuối cụm từ tiếng Anh, dịch trước, rồi mới dịch ngược lên các từ bổ nghĩa phía trước.
    *   **Khi không có Thuật ngữ Việt rõ ràng:** Giữ nguyên tiếng Anh gốc. Cân nhắc thêm một giải thích ngắn gọn bằng tiếng Việt trong ngoặc đơn ở lần xuất hiện đầu tiên (VD: "...sử dụng phương pháp gradient descent (kỹ thuật tối ưu dựa trên đạo hàm)...").

*   **4.3. Xử lý Viết tắt nói chung (Khi không có trong Mục 4.1):**
    *   Nếu văn bản gốc viết: "Deep Neural Network (DNN)", cố gắng dịch: "Mạng Nơ-ron Sâu (DNN)". Sau đó ưu tiên dùng "DNN" cho toàn bài.
    *   Các tổ chức/khái niệm quốc tế phổ biến (UNESCO, WHO, API, AI, CEO...) luôn ưu tiên giữ nguyên viết tắt gốc, không dịch dài dòng.

*   **4.4. Xử lý Trích dẫn, Tiêu đề Khoa học & Danh pháp:**
    *   **In-text Citations:** Bảo toàn nguyên vẹn định dạng trích dẫn (VD: `[1, 3-5]`, `(Smith et al., 2021)` dịch thành `[1, 3-5]`, `(Smith và cộng sự, 2021)`).
    *   **Captions:** Chuẩn hóa các tiền tố tiêu đề: `Figure/Fig.` -> `Hình`; `Table` -> `Bảng`; `Equation/Eq.` -> `Phương trình`.
    *   **Danh pháp khoa học:** Giữ nguyên tên gốc (Tiếng Latin/Tiếng Anh) đối với sinh vật, hợp chất hóa học, trừ khi đã có tên Việt hóa cực kỳ phổ biến.
    
*   **4.5. Nhất quán Tuyệt đối:** 
    *   Dù thuật ngữ lấy từ Mục 4.1 hay do bạn tự dịch (Mục 4.2), một khi đã chốt cách dịch ở đoạn đầu tiên, **PHẢI giữ thống nhất 100% cách dịch đó cho đến cuối tài liệu**. Tuyệt đối không dịch một từ thành hai nghĩa khác nhau ở hai vị trí khác nhau.
